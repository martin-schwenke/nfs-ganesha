			       ...
			       struct fsal_attrlist *attrib)
{
	...
	//FIXME: Intentionally left here to ponder whether this should
	//be a no-op (as above) or a warning (as below)
	if (!attrib->acl) {
	...
	if (ret != 0) {
		status = fsalstat(errno, 0);
		LogMajor(COMPONENT_FSAL, "failed to set default type posix acl");
		goto out;
	}
